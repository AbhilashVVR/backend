...
const addAnalytics = async () => {
  ...
  params.Item.updatedAt = formatInTimeZone(
    new Date(),
    "Asia/Kolkata",
    "yyyy-MM-dd HH:mm:ssXXX"
  );
  params.Item.lastPaymentAt = formatInTimeZone(
    new Date(),
    "Asia/Kolkata",
    "yyyy-MM-dd HH:mm:ssXXX"
  );
...
const editAnalyticsById = async (details) => {
  ...
  const lastPayment = format(
    new Date(previousAnalytics.lastPaymentAt),
    DATE_FORMAT
  );
  ...
  const totalUser = details.totalUser
    ? previousAnalytics.totalUser + 1
    : previousAnalytics.totalUser;
  const verfiedUser = details.verfiedUser
    ? previousAnalytics.verfiedUser + 1
    : previousAnalytics.verfiedUser;
  const guestUser = details.guestUser
    ? previousAnalytics.guestUser
    : previousAnalytics.guestUser + 1;
  const newUser = startDate === lastUpdate ? previousAnalytics.newUser + 1 : 1;
  const totalPayment = details.totalPayment
    ? previousAnalytics.totalPayment + details.totalPayment
    : previousAnalytics.totalPayment;
  const lastPaymentAt = details.totalPayment
    ? updatedAt
    : previousAnalytics.lastPaymentAt;
  const dailyPayment =
